...
- 🚨 CRITICAL — any of: unpatched CVE in installed tooling; dangerouslyAllowAllBuilds: true; npm ignore-scripts absent (scripts run by default — primary attack vector); release age not configured on any manager; npm release-age key spelled so npm ignores it (gate inactive); Yarn <4.12 with a duration-string release-age value (parsed as minutes — a 7d gate is 7 minutes); minimumReleaseAgeExclude set without minimumReleaseAge (false security posture); lockfile gitignored. Do not use for optional hardening gaps.
